...
COMPANY_CONTACT_DONE_STATUSES = {"contacts_generated", "no_new_contacts"}
...
COMPANY_ALIASES = {
    ...
    "source_url": ["source_url", "audit_url", "entry_url", "website_url", "website", "Website"],
    "evidence": ["evidence", "gap_reason", "verified_gaps", "broken_funnel_pages"],
...
class ApolloDecisionMakerFinder:
    ...
    async def find(self, company: dict[str, str], titles: list[str], limit: int) -> list[Contact]:
        ...
        for person in people[: max(limit * 3, limit)]:
            ...
            if contact := apollo_contact(enriched or person, domain):
...
def qualified_company(
    row: dict[str, str],
    qualified_statuses: set[str],
    qualified_confidences: set[str] | None = None,
) -> bool:
    if truthy(first_value(row, COMPANY_ALIASES["do_not_sequence"])):
        return False
    confidence = first_value(row, COMPANY_ALIASES["confidence"]).casefold()
    if qualified_confidences is not None and confidence not in qualified_confidences:
        return False
...
def should_skip_for_guardrail(row: dict[str, str], retry_failed: bool, force: bool) -> bool:
    if force:
        return False
...
def contact_row(
    ...
    return {
        ...
        "solution_angle": company.get("solution_angle", "") or company.get("outreach_reason", ""),
        "technologies": company.get("technologies", ""),
        "selected_outreach_tools": company.get("selected_outreach_tools", ""),
        "automation_opportunity_1": company.get("automation_opportunity_1", ""),
        ...
        "audit_company_key": company_key(company),
...
async def extract_contacts(
    ...
    configured_confidences = config.get("qualified_confidences")
    qualified_confidences = (
        {str(confidence).casefold() for confidence in configured_confidences}
        if configured_confidences is not None
        else None
    )
    titles = [str(title) for title in config["decision_maker_titles"]]
    ...
    for source_row in companies_sheet.rows():
        ...
        if not qualified_company(source_row.data, qualified_statuses, qualified_confidences):
            continue
        summary["companies_qualified"] += 1
        if should_skip_for_guardrail(source_row.data, retry_failed=retry_failed, force=force):
