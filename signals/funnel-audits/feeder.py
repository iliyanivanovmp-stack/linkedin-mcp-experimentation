...
DEFAULT_CONFIG = Path(__file__).with_name("feeder_config.json")
...
def is_campaign_duplicate(error: str) -> bool:
    normalized = error.casefold()
    return (
        "lead already in the campaign" in normalized
        or is_cross_campaign_duplicate(normalized)
    )
...
def process(
    ...
    for source in config["sources"]:
        ...
        # Heal duplicate responses that older runs recorded as hard failures.
        # This happens before readiness and daily-limit checks because no new
        # Lemlist lead is created by the reconciliation.
        for row in rows:
            if row.data.get("status", "").strip().casefold() != "failed":
                continue
            error = str(row.data.get("lemlist_error", "") or "")
            if not is_campaign_duplicate(error):
                continue
        ...
        for row in rows:
            ...
            try:
                ...
                if is_campaign_duplicate(error):
                    if not dry_run:
                        sheet.update_row(row.number, {
                            **base_update,
                            "status": "skipped_existing_campaign",
...
def main() -> None:
    ...
    try:
        ...
        sys.exit(1)
    source_errors = any(source.get("error") for source in result.get("sources", []))
    status = "partial_failure" if result.get("failed") or source_errors else "success"
