...
LINKEDIN_ONLY_CAMPAIGN = "Pipeline Engine hiring outreach - LinkedIn only"
PLACEHOLDER_EMAIL_DOMAIN = "pipeline-engine-hiring-linkedin-only.invalid"
...
def start_enrichment(sheet: Sheet, client: LemlistEnrichmentClient, dry_run: bool, limit: int | None) -> dict[str, Any]:
    ...
    try:
        ...
        # Lemlist enrichment is an optional first provider. Route the batch to
        # Apollo instead of aborting the complete daily pipeline and starving
        # the feeder of contacts that already have valid email addresses.
        error = str(exc)[:1000]
...
def apollo_enrichment(sheet: Sheet, client: ApolloEnrichmentClient, dry_run: bool, limit: int | None) -> dict[str, Any]:
    ...
    for row in sheet.rows():
        ...
        summary["rows_seen"] += 1
        data = row.data
        if data.get("status", "").strip().casefold() != "email_not_found":
            continue
        if data.get("email", "").strip():
            continue
        summary["checked"] += 1
...
def finalize_linkedin_only(
    ...
    eligible_statuses = {"email_not_found", "apollo_email_not_found", "email_enrichment_failed"}
    for row in sheet.rows():
