...
def retry_due(row: dict[str, str], max_attempts: int, now: datetime | None = None) -> bool:
    if row.get("status", "").strip().casefold() != "delivery_failed":
        return False
    try:
        attempts = int(row.get("delivery_attempts", "0") or 0)
    except ValueError:
        attempts = 0
    if attempts >= max_attempts:
        return False
    retry_at = row.get("delivery_next_retry_at", "").strip()
    if not retry_at:
        return True
    try:
...
def process(
    ...
    retry_config = config.get("delivery_retry", {})
    max_delivery_attempts = int(retry_config.get("max_attempts", 4))
    retry_after_minutes = int(retry_config.get("retry_after_minutes", 60))
    max_attempts_per_run = int(config.get("max_attempts_per_run", 90))
    summary = {
        "dry_run": dry_run,
        "sources": [],
        "ready": 0,
    ...
    for source in config["sources"]:
        ...
        for row in rows:
            if limit is not None and summary["plugged"] >= limit:
                break
            if daily_remaining is not None and source_result["plugged"] >= daily_remaining:
                break
            if summary["attempted"] >= max_attempts_per_run:
                break
            status = row.data.get("status", "").strip()
            if status and not retry_due(row.data, max_delivery_attempts):
                continue
            ...
            if missing:
                mark_failed(sheet, row.number, base_update, f"Missing: {', '.join(missing)}", dry_run, retryable=False)
                summary["failed"] += 1
            ...
            if not campaign_id and not dry_run:
                mark_failed(sheet, row.number, base_update, "Missing Lemlist campaign ID", dry_run, retryable=False)
                summary["failed"] += 1
                source_result["failed"] += 1
                continue
            if client is None and not dry_run:
                mark_failed(sheet, row.number, base_update, "Missing LEMLIST_API_KEY", dry_run, retryable=True, retry_after_minutes=retry_after_minutes, previous=row.data)
                summary["failed"] += 1
                source_result["failed"] += 1
                continue
            ...
            try:
                ...
                if is_cross_campaign_duplicate(error):
                    ...
                    mark_failed(sheet, row.number, base_update, error, dry_run, retryable=True, retry_after_minutes=retry_after_minutes, previous=row.data)
                    summary["failed"] += 1
                    source_result["failed"] += 1
...
def mark_failed(
    ...
    if dry_run:
        return
    attempts = int((previous or {}).get("delivery_attempts", "0") or 0) + 1
    now = datetime.now(timezone.utc)
    sheet.update_row(row_number, {
        **base_update,
        "status": "delivery_failed" if retryable else "failed",
        "lemlist_error": error[:1000],
        "delivery_attempts": str(attempts),
        "delivery_last_attempt_at": now.isoformat(),
        "delivery_next_retry_at": (now + timedelta(minutes=retry_after_minutes)).isoformat() if retryable else "",
    })
